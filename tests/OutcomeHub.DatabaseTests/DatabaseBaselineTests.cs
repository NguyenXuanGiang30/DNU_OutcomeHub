...
public sealed class DatabaseBaselineTests
{
    ...
    public async Task FreshDatabaseAppliesCanonicalMigrationsAndPassesCourseRlsSmokeTest()
    {
        ...
        Assert.Equal(19, concurrentResults.Sum(result => result.AppliedCount));
        Assert.Equal(19, concurrentResults.Sum(result => result.SkippedCount));
        Assert.Equal(0, noOpResult.AppliedCount);
        Assert.Equal(19, noOpResult.SkippedCount);
        ...
        CancellationToken cancellationToken)
    {
        ...
        Assert.Equal(19L, reader.GetInt64(3));
