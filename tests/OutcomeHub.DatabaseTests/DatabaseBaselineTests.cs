...
public sealed class DatabaseBaselineTests
{
    ...
    public async Task FreshDatabaseAppliesCanonicalMigrationsAndPassesCourseRlsSmokeTest()
    {
        ...
        Assert.Equal(14, concurrentResults.Sum(result => result.AppliedCount));
        Assert.Equal(14, concurrentResults.Sum(result => result.SkippedCount));
        Assert.Equal(0, noOpResult.AppliedCount);
        Assert.Equal(14, noOpResult.SkippedCount);
