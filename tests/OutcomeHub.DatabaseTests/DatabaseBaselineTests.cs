...
public sealed class DatabaseBaselineTests
{
    ...
    public async Task FreshDatabaseAppliesCanonicalMigrationsAndPassesCourseRlsSmokeTest()
    {
        ...
        Assert.Equal(16, concurrentResults.Sum(result => result.AppliedCount));
        Assert.Equal(16, concurrentResults.Sum(result => result.SkippedCount));
        Assert.Equal(0, noOpResult.AppliedCount);
        Assert.Equal(16, noOpResult.SkippedCount);
        ...
        CancellationToken cancellationToken)
    {
        ...
        Assert.Equal(16L, reader.GetInt64(3));
